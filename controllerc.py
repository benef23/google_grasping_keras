...
@nrp.MapRobotPublisher("joint1", Topic("/robotc/arm_1_joint/cmd_pos", std_msgs.msg.Float64)) 
@nrp.MapRobotPublisher("rewardc_ros", Topic("/rewardc", std_msgs.msg.Float64))
@nrp.MapRobotSubscriber("links", Topic("/gazebo/link_states", gazebo_msgs.msg.LinkStates)) 
@nrp.MapRobotPublisher("joint4", Topic("/robotc/arm_4_joint/cmd_pos", std_msgs.msg.Float64))
@nrp.MapRobotPublisher("joint2", Topic("/robotc/arm_2_joint/cmd_pos", std_msgs.msg.Float64))
@nrp.MapRobotPublisher("joint5", Topic("/robotc/arm_5_joint/cmd_pos", std_msgs.msg.Float64))
@nrp.MapRobotPublisher("joint3", Topic("/robotc/arm_3_joint/cmd_pos", std_msgs.msg.Float64))
@nrp.MapRobotSubscriber("joints", Topic("/jointState", sensor_msgs.msg.JointState))
@nrp.MapVariable("agent", initial_value=None)
@nrp.MapVariable("session3", initial_value=None)
@nrp.MapVariable("graph3", initial_value=None)
#actuators


@nrp.Neuron2Robot()
def controllerc (t, joints, links, joint2, joint3, joint4, joint5, rewardc_ros, joint1, agent, graph3, session3):
    ...
    # create the nets for rl agent
     # actor net
        ...
        graph3.value = Graph()
        with graph3.value.as_default():
                session3.value = Session()
                with session3.value.as_default():
...
#### run steps
   ...
   #  clientLogger.info('humerus_angle ', links.value.pose[15].position.y)  
      ...
      #  clientLogger.info('radius_ang_vel ', angle_vel_lower)
                ...
                rewardc_ros.send_message(reward)
    ...
    #-(angle_lower**2 + 0.1*angle_vel_lower**2 +
        ...
        #learn from the reward
                ...
                if agent.value.step%20 == 0:
                    ...
                    PATH = '/home/user/Desktop/keras_learning_weights/ddpg_weights_c.h5'
